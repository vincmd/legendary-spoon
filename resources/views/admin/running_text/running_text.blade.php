...
<body class="h-[99.8vh] w-full flex flex-col justify-center items-center colorback">
        ...
        <section class="ml-10 w-full h-19/20 flex items-center flex-col">
            ...
            <div class="bg-white border-1 rounded border-black h-7/8 w-5/8 flex flex-col items-center justify-around ">
                ...
                <div
                    ...
                    <form action="{{ route('update_text') }}" method="post" class="w-full h-full">
                        @csrf
                        <p id="textdisplay" class="w-full h-full font-sans text-lg font-normal ">
                            @if (session('test'))
                                {{ $running_text->texts }}
                            @endif
                        </p>
    ...
    {{-- paragraf to input --}}
    <script>
        document.getElementById('textdisplay').addEventListener('dblclick', editText);

        function editText() {
            const para = document.getElementById('textdisplay');
            ...
            input.name = 'text'
            input.className =
                'w-full h-full font-sans text-lg font-normal text-start flex flex-col items-left justify-start max-w-full break-words';
            para.replaceWith(input);
            input.focus();
            var buttonz = document.createElement('button');
            buttonz.onclick = function() {
                window.location.href = '/admin/running_text';
            };
            buttonz.className = 'bg-yellow-400 w-18 h-8 border border-black rounded-lg ';
            buttonz.innerText = 'cancel';
            document.getElementById('info').replaceWith(buttonz);
            document.getElementById('sub_but').classList.remove('hidden');
        }
    </script>
