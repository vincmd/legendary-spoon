...
<body class="h-[99.8vh] w-full flex flex-col justify-center items-center colorback">
    ...
    <main class="h-full w-full flex flex-row">
        <nav class=" h-full w-1/6 flex flex-row justify-arround colorback" id="navigator">
            <div class="flex bgcolorside flex-col justify-start items-center h-full z-2">
                <div class="w-20 h-1/2 sticky flex flex-col justify-evenly items-center z-[2] pl-2" style="z-index: 2">
            ...
            <div class="flex flex-col items-start  bgcolorside pr-0" id="navtests">
        ...
        <section class="ml-10 w-full flex flex-col items-center">
            ...
            <div class=" h-12 flex flex-row justify-center items-center">
                <h1 class="font-bold text-2xl font-sans">Loket</h1>
            </div>
            ...
            <div class="bg-white h-auto min-h-68 w-9/10 rounded-sm  flex flex-col items-center gap-7">
                <div class="w-35/36 h-20 mt-3 flex flex-row justify-between px-4 items-center">
                    <div class="flex flex-col w-30 items-center h-full justify-between ">
                        <h1 class="text-xl font-sans font-semibold">Data locket</h1>
                        <button class="bg-green-400 w-full h-8 rounded-lg"
                            onclick="window.location.href='/admin/locket/new';">add locket</button>
                    </div>
                    <div class="relative">
                        <form action="{{ route('search_locket') }}" method="get" >
                            @csrf
                ...
                <table class="table w-35/36 h-auto ">
                    <thead class="bgcolormain w-full h-8 ">
                        <tr>
                            ...
                            <th class="border border-black text-lg colormain">alias</th>
                            <th class="border border-black text-lg colormain">user</th>
                        ...
                        @foreach ($lockets as $locket)
                            <tr>
                                <td class="border border-black font-mono font-semibold text-center">
                                    {{ $loop->iteration }}</td>
                                <td class="border border-black font-mono font-semibold pl-1">{{ $locket->alias }}</td>
                                <td class="border border-black font-mono font-semibold pl-1">{{ $locket->email_user }}
                                </td>
                                <td class="border border-black font-mono font-semibold pl-1">
                                    <form method="POST" action="{{ route('locket.destroy', $locket->id) }}"
    ...
    <script>
        function hideoropen() {
            const navigators = document.getElementById('navtests');
