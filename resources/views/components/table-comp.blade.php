...
@php
    ...
    $withSClass = 'App\\Models\\' . ucfirst($tableSegment) . 's';

    if (class_exists($baseClass)) {
        $modelClass = $baseClass;
    } elseif (class_exists($withSClass)) {
        $modelClass = $withSClass;
    } else {
        // Handle error: model class not found
        abort(404, "Model for '{$tableSegment}' not found.");
    }

    // Now instantiate the model and get table columns
    $modelInstance = new $modelClass();
    $tableName = $modelInstance->getTable();

    $keysToUnset = ['created_at', 'updated_at'];
    foreach ($un_use_col as $key) {
        $keysToUnset[] = $key;
    }

    $columns = Schema::getColumnListing($tableName);

    foreach ($custom_col as $original => $customLabel) {
        $index = array_search($original, $columns);
        if ($index !== false) {
            $columns[$index] = $customLabel;
        }
        // dd($col_name);
    }
    $keysToUnset_head = $keysToUnset;
    $keysToUnset_head[] = 'id';
    $col_name = array_diff($columns, $keysToUnset_head);
    //  to remove keys from data rows :
    ...
    $destroy_name = $exploded_url[2] . '.destroy';
                ...
                @if (!$table_data->isEmpty())
                    @foreach ($table_data as $items)
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3 text-sm">
                                <form action="{{ route($destroy_name, $ids) }}" method="post"
                                    class="flex flex-row justify-start items-center ">
                                    @csrf
                                    @method('delete')
                                    <button type="submit" class="ml-2 flex flex-row justify-start items-center">
